...
const validate = function () {
    ...
    if (password.value === '' || password.value.length < 8) {
        failures.push({input: 'input-password', msg: 'Must be at least 8 character'})
    }
    ...
    if (!chk.checked) {
        failures.push({input: 'input-alive', msg: 'No undead allowed'})
    }
    return failures;
};
...
const send = function(ev) {
    ev.preventDefault();
    ev.stopPropagation(); // prevents bubbbling up to any parent element (the click)

    let fails = validate()

    if(fails.length === 0) {
        document.getElementsById('form-user').submit();
    } else {
        fails.forEach(obj => {
